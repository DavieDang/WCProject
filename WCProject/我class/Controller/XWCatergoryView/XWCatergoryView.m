...
- (void)xwp_interpolationForBottomLineWithRatio:(CGFloat)ratio{
    if (!_bottomLineEable || !_data.count) return;
    CGFloat centerX = [self xwp_interpolationFromValue:_fromModel.cellCenter.x toValue:_toModel.cellCenter.x ratio:ratio - (int)ratio];
    CGFloat y = CGRectGetMaxY(_fromModel.cellFrame) + _bottomLineSpacingFromTitleBottom;
    CGFloat height = _bottomLineWidth;
    _bottomLine.bounds = CGRectMake(0, 0,[UIScreen mainScreen].bounds.size.width/2, height);
    _bottomLine.center = CGPointMake(centerX, y + height / 2.0f);
}
...
- (void)xwp_updateBottomLineWhenClickedWithIndexPath:(NSIndexPath *)indexPath{
    ...
    XWCatergoryViewCellModel *model = _data[indexPath.item];
    [UIView animateWithDuration:_clickedAnimationDuration animations:^{
        _bottomLine.bounds = CGRectMake(0, 0, 100, _bottomLine.height);
        _bottomLine.center = CGPointMake(model.cellCenter.x, _bottomLine.y + _bottomLine.height / 2.0f);
    }];
}
